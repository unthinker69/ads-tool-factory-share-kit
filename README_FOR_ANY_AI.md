...
4. `docs/PRODUCT_INTAKE_FORM.md`
5. The target product's `product.json`, `brief.md`, and `TASK.md`
...
3. Build product-specific configuration, prompt rules, UI defaults, API schema, scoring guardrails, and release checks.
4. Derive UI color, typography direction, examples, and default states from the provided brand/product materials.
5. Reuse only the modules that are listed in `docs/distribution_manifest.md`.
6. Record local branch optimizations in `docs/branch_optimizations.md`.
7. If the local owner explicitly approves distribution within their own factory, add the item to `docs/distribution_queue.md`.
8. Run the release checklist and report verification.
